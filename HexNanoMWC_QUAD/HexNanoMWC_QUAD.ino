...
void setup()
{
  ...
  RGY_GRN_ON;                                             //Start At Full Health
...
void loop () {
  ...
  if (f.ARMED && isLaunch) {
    ...
    if (!accReached) {
      ...
      if (rcData[THROTTLE] < MAXTHROTTLE) {
        rcData[THROTTLE] = startThrottle;
        if (startThrottle <= launchThrot) {
          startThrottle += incThrottle;
        }
      }
      
      if (EstAlt - altAtLaunch > 30){
        accReached = true;
        launchTime = micros();
      }
    }
    else{
      estAltAtHoverInit = EstAlt;
      rcData[THROTTLE] = rcData[THROTTLE] - 295; // Try a percentage
  ...
  if (currentTime > rcTime ) { // 50Hz
    ...
    #if BARO 
      #if (!defined(SUPPRESS_BARO_ALTHOLD))// CONTROL IT!
        if (rcOptions[BOXBARO] || launchCalledHover) {
            if (!f.BARO_MODE) {
              ...
              BaroPID=0;
                ...
                error = estAltAtHoverInit - EstAlt;
                if(error > 0){
                    rcData[THROTTLE] = rcData[THROTTLE] + .75*error;
                }
                else{
                  rcData[THROTTLE] = rcData[THROTTLE] + .25*error;
                }
                
              
              if (rcCommand[THROTTLE] > curThrot || rcCommand[THROTTLE] >= 2000){
                launchCalledHover = false;
              }
 ...
 //*********************************** 
      ...
      static uint8_t isAltHoldChanged = 0;
        ...
        if (abs(rcCommand[THROTTLE]-initialThrottleHold) > ALT_HOLD_THROTTLE_NEUTRAL_ZONE) {
         ...
         // initialThrottleHold += (rcCommand[THROTTLE] > initialThrottleHold) ? -ALT_HOLD_THROTTLE_NEUTRAL_ZONE : ALT_HOLD_THROTTLE_NEUTRAL_ZONE;; //++hex nano
          ...
          rcCommand[THROTTLE] = initialThrottleHold + BaroPID; // THIS IS WHERE CONTROL HAPPENS
  ...
  switch(n){
    ...
    case 2:
    {
      if((micros() - lastMove) > 5000000 ){
        if (rand()%2 == 1){
          rcData[THROTTLE] = rcData[THROTTLE] - 250;
        }
        else{
          rcData[THROTTLE] = rcData[THROTTLE] + 250;
        }
    ...
    case 3:
    {
        if((micros() - lastMove) > 5000000 ){
          if (rand()%2 == 1){
            rcData[THROTTLE] = rcData[THROTTLE] - 500;
          }
          else{
            rcData[THROTTLE] = rcData[THROTTLE] + 500;
          }
